...
"""
Run search without session, so output to folder as normal.
"""
dynesty = af.DynestyStatic(
    path_prefix=path.join("database", "session_after_run", dataset_name),
    number_of_cores=1,
)
...
"""
session = af.db.open_database(path.join("output", "session.sqlite"))

"""
Does running now its completed put it in the database?
"""
dynesty = af.DynestyStatic(
    path_prefix=path.join("database", "session_after_run", dataset_name),
    number_of_cores=1,
