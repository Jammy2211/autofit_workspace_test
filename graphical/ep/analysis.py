...
class Analysis(af.Analysis):
    ...
    def model_data_from_instance(self, instance):
        ...
        If the `instance` is not a `Collection` and thus contains just one profile, we raise a TypeError and return
        the profile for that single model component.
        """
        ...
        try:
            return sum(
                [
                    profile.profile_1d_via_xvalues_from(xvalues=xvalues)
                    for profile in instance
                ]
            )
        except TypeError:
            return instance.profile_1d_via_xvalues_from(xvalues=xvalues)
