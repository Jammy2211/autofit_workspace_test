...
for model_index in range(len(data_list)):
    ...
    gaussian.centre = af.LogUniformPrior(
        lower_limit=1e-2, upper_limit=100.0
    )
    gaussian.normalization = af.LogUniformPrior(lower_limit=1.0, upper_limit=10.0)
    gaussian.sigma = af.LogUniformPrior(lower_limit=5.0, upper_limit=15.0)
...
paths = af.DirectoryPaths(
    name=path.join(
        "ep", "hierarchical", "factor_gaussian_bounded", "log_uniform_priors"
    )
)
