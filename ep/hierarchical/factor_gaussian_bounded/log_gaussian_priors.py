...
for model_index in range(len(data_list)):
    ...
    gaussian.centre = af.LogGaussianPrior(
        mean=50.0, sigma=20.0,
    )
    gaussian.normalization = af.LogGaussianPrior(mean=3.0, sigma=5.0)
    gaussian.sigma = af.LogGaussianPrior(mean=10.0, sigma=10.0)
...
paths = af.DirectoryPaths(
    name=path.join(
        "ep", "hierarchical", "factor_gaussian_bounded", "gaussian_priors"
    )
)
