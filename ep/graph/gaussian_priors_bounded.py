...
for model_index in range(len(data_list)):
    ...
    gaussian.normalization = af.GaussianPrior(mean=3.0, sigma=5.0, lower_limit=0.0, upper_limit=100.0)
    gaussian.sigma = af.GaussianPrior(mean=10.0, sigma=10.0, lower_limit=0.0, upper_limit=25.0)
...
paths = af.DirectoryPaths(
    name=path.join(
        "ep", "graph", "gaussian_priors_bounded"
    )
)
